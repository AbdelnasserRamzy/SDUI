...
extension SlideshowViewModel {
    ...
    var timer: TimePublisher? {
        ...
        return Timer.publish(every: 3, on: .main, in: .common).autoconnect()
    }
    ...
    func itemWidth(_ item: Data.Element) -> CGFloat {
        guard activeIndex < data.count else {
            return 0
        }
        let activeItem = data[activeIndex as! Data.Index]
        return activeItem[keyPath: _dataId] == item[keyPath: _dataId] ? UIScreen.main.bounds.width-100 : 50
    }
}
